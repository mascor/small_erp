...
class User(UserMixin, db.Model):
    ...
    # Internal hourly cost rate for timesheet-based cost calculation
    hourly_cost_rate = db.Column(db.Numeric(10, 2), nullable=True, default=Decimal('0'))
...
class RevenueActivity(db.Model):
    ...
    timesheets = db.relationship('TimesheetEntry', backref='activity', lazy='dynamic', cascade='all, delete-orphan')
...
class ActivityCost(db.Model):
    ...
    # --- New fields for timesheet integration ---
    # line_type: generic | internal_consultant | external_consultant
    line_type = db.Column(db.String(30), nullable=False, default='generic')
    # source_type: manual | timesheet_aggregate
    source_type = db.Column(db.String(30), nullable=False, default='manual')
    # FK to user when line_type = internal_consultant
    source_user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)
    # Vendor name for external_consultant costs
    vendor_name = db.Column(db.String(200), nullable=True)
    # Aggregated hours (used when source_type = timesheet_aggregate)
    hours_total = db.Column(db.Numeric(10, 2), nullable=True)
    # Unit rate snapshot (used when source_type = timesheet_aggregate)
    unit_rate = db.Column(db.Numeric(10, 2), nullable=True)
    # True when this row was auto-generated by the timesheet engine
    ...
    LINE_TYPES = [
        ('generic', 'Generico'),
        ('internal_consultant', 'Consulente interno'),
...
class TimesheetEntry(db.Model):
    ...
    # Snapshot of hourly rate at time of entry — never recalculated from current rate
    hourly_rate_snapshot = db.Column(db.Numeric(10, 2), nullable=False)
    created_by = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    user = db.relationship('User', foreign_keys=[user_id], backref='timesheet_entries')
    creator = db.relationship('User', foreign_keys=[created_by])

    def __repr__(self):
        return f'<TimesheetEntry user={self.user_id} activity={self.activity_id} date={self.work_date} hours={self.hours}>'
...
class AuditLog(db.Model):
    ...
    ACTION_TYPES = [
        'login', 'logout', 'read', 'create', 'update', 'delete', 'status_change',
