...
def calc_activity_totals(activity):
    ...
    try:
        ...
        # Query by activity id to work reliably even when the input object
        # comes from a different session in tests or background jobs.
        ...
        logger.debug(f'Activity totals calculated: activity_id={activity.id}, revenue={revenue}, total_costs={total_costs}, net_margin={net_margin}')
        return {
            'revenue': revenue,
...
def calc_dashboard_stats():
    ...
    try:
        ...
        logger.debug(f'Dashboard stats calculated: open_activities={open_activities}, month_revenue={month_revenue}, month_margin={month_margin}')
        return {
